...
.weather-cell {
  ...
  border-radius: 14px; padding: 6px 6px; min-height: 110px;
}
.weather-cell .label { font-size: .85rem; opacity: .8; margin-bottom: .25rem; }
.weather-cell .temp { font-size: 1.1rem; margin-top: .35rem; }
.weather-cell .icon { width: 48px; height: 48px; color:#e7eaee; }
.weather-cell .icon svg, .weather-cell .icon img { width:48px; height:48px; display:block; }
...
# ankkuri CSS-kohdistusta varten
...
with col_names:
    try:
        ...
        # --- Päivämäärä ja viikonpäivä suomeksi (otsikkoon) ---
        ...
        <section class="card" style="
          ...
          <div class="card-body" style="display:flex; justify-content:center; align-items:center; text-align:center; flex:1; position:relative; z-index:2;">
            <div style="font-size:1.35rem; font-weight:800; margin:0 0 6px 0;">{names}</div>
          </div>

          <!-- Perhonen taustana oikealla: contain + right center -->
          {'<div style="position:absolute; inset:0; background-image:url('+BG_DATAURL+'); background-repeat:no-repeat; background-size:contain; background-position:right center; pointer-events:none; filter: drop-shadow(0 6px 16px rgba(0,0,0,.45)); z-index:0;"></div>' if BG_DATAURL else ''}

          <!-- Hento tumma liuku päälle, jotta teksti pysyy luettavana -->
          <div style="position:absolute; inset:0; background:linear-gradient(90deg, rgba(11,15,20,0.65) 0%, rgba(11,15,20,0.25) 45%, rgba(11,15,20,0.00) 70%); pointer-events:none; z-index:1;"></div>
        </section>
        """
