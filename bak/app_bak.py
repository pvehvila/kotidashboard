...
@st.cache_data(ttl=300)
def fetch_btc_last_12h_eur():
    """
    Hakee Bitcoinin hinnat viimeiseltä 12 tunnilta (EUR) CoinGeckosta.
    Käyttää /market_chart/range -päätettä, joka ei vaadi API-avainta.
    ...
    # Harvennetaan 5 min välein, jotta kuvaaja pysyy kevyenä
    pruned = []
    last_added = None
    for t, p in out:
        if (last_added is None) or ((t - last_added).total_seconds() >= 300):
            pruned.append((t, p))
            last_added = t
...
<body>
  <div class="title">Sää – Riihimäki</div>
...
with st.container():
    try:
        ...
        if df12.empty:
            card("Pörssisähkö – seuraavat 12 h", "<span class='hint'>Ei hintoja saatavilla.</span>", height_dvh=22)
        else:
            ...
            fig.update_layout(
                title_text="Pörssisähkö – seuraavat 12 h",
                title_x=0,
                title_font_size=14,
...
""", unsafe_allow_html=True)
        ...
        card("Pörssisähkö – seuraavat 12 h", f"<span class='hint'>Virhe hinnanhaussa: {e}</span>", height_dvh=22)
...
with col_names:
    try:
        ...
        """
        card("Nimipäivät", body, height_dvh=16)
    except Exception as e:
...
# Kevyt "zen"-tausta: vuorilinja-aaltojen siluetti (himmeä)
        ...
        ZEN_BG_DATAURL = None
        try:
            img_path = os.path.join(os.path.dirname(__file__), "zen-bg.png")  # sama hakemisto kuin app.py
...
try:
    ...
    <div style="display:flex; justify-content:space-between; align-items:flex-end; gap:10px; margin-bottom:6px;">
      <div style="font-size:1.35rem; font-weight:800">Bitcoin</div>
    </div>
    """

    # <div style="font-size:1.35rem; font-weight:800">Bitcoin: {eur_now_fmt} €</div>

    # Piirretään 12 h käyrä + ATH-viiva
    fig = go.Figure()
    ...
    if ath_eur:
        fig.add_trace(go.Scatter(
            ...
            line=dict(dash="dot"),
            hovertemplate="ATH — %{y:.0f} €<extra></extra>",
        ))
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            title=None,
            gridcolor="rgba(255,255,255,0.28)",     # selkeämpi ruudukko
            tickformat="%H:%M",
    ...
    st.plotly_chart(fig, use_container_width=True, theme=None, config={"displayModeBar": False})
    st.markdown(
        "<div class='hint' style='margin-top:4px;'>Näytetään viimeiset 12 h (CoinGecko), katkoviiva = ATH{}</div>".format(
    ...
    # card("Bitcoin (EUR)", top_html + st_plotly_html, height_dvh=16)
