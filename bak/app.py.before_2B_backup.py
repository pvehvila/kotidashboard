...
def _quote_from_zenquotes():
    """Fetch one quote from ZenQuotes. Returns dict with 'q' and 'a' keys or None."""
    ...
    try:
        ...
        if r.ok:
            ...
            if isinstance(data, list) and data:
                item = data[0]
                return {"text": item.get("q"), "author": item.get("a")}
    except Exception:
        pass
    return None

@st.cache_data(ttl=600, show_spinner=False)
def _quote_from_quotable():
    """Fetch one quote from Quotable. Returns dict with 'text' and 'author' or None."""
    import requests
    try:
        r = requests.get("https://api.quotable.io/random?tags=wisdom|inspirational", timeout=10)
        if r.ok:
            item = r.json()
            return {"text": item.get("content"), "author": item.get("author")}
    except Exception:
        pass
    return None
